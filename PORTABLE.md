...
Extract `lit-rip-linux-x86_64-v0.2.2.tar.gz` to a local Linux folder (the archive preserves executable permissions). Run the `launch-lit-rip.sh` inside the extracted folder, or use its full path as a desktop launcher's command. The browser app opens automatically. Keep the app running while you use it.
